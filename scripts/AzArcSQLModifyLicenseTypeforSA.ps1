...
# =============================================================================
#
# BASED ON: Official Microsoft script modify-arc-sql-license-type.ps1 v3.0.5
# Source   : https://github.com/microsoft/sql-server-samples/blob/master/samples/manage/azure-arc-enabled-sql-server/modify-license-type/
#
# RBAC MINIMUM REQUIREMENTS (Azure Automation - Managed Identity):
#   The Automation Account's Managed Identity needs the following roles on each
#   target subscription (or resource group):
#
#   1. Reader
#      - Actions: */read
#      - Purpose: List subscriptions, resource groups, and Arc machines
#      - az role assignment create --assignee <MI-ObjectId> \
#          --role "Reader" --scope /subscriptions/<SubId>
#
#   2. Azure Connected Machine Resource Administrator
#      - Actions: Microsoft.HybridCompute/machines/read,
#                 Microsoft.HybridCompute/machines/extensions/read,
#                 Microsoft.HybridCompute/machines/extensions/write
#      - Purpose: Read Arc machines and update SQL extension settings (LicenseType)
#      - az role assignment create --assignee <MI-ObjectId> \
...
# Azure Automation nao suporta caminhos relativos; usa $env:TEMP se disponivel
$transcriptPath = if ($env:TEMP) { Join-Path $env:TEMP 'modify-arc-sql-license-type.log' } else { '.\modify-arc-sql-license-type.log' }
try { Start-Transcript -Path $transcriptPath -ErrorAction Stop } catch { Write-Output "Start-Transcript falhou (esperado em Azure Automation sandbox): $($_.Exception.Message)" }
