...
try {
    ...
    # Seleciona os primeiros 5 endpoints
    $topFiveEndpoints = $dynamicEndpoints | Select-Object -First 5
    "Top 5 dynamic endpoints: $($topFiveEndpoints -join ', ')" | Out-File -FilePath $logFilePath -Append 
}
catch {
...
$allEndpoints = $staticEndpoints + $topFiveEndpoints
...
# Combine static endpoints with the allowed dynamic endpoints for HTTP requests
$finalEndpointsForRequest = $allowedEndpoints
